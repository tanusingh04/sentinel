...
import { ALERTS, sortByPriority, type Priority, type Status } from "@/lib/mock-alerts";
...
const priorities: (Priority | "all")[] = ["all", "critical", "medium", "low"];
const statuses: (Status | "all")[] = ["all", "pending", "in_progress", "resolved"];
...
function AlertsPage() {
  ...
  const filtered = useMemo(() => {
    return sortByPriority(alerts).filter((a) => {
      ...
      if (q && !(`${a.title} ${a.device} ${a.system} ${a.id}`.toLowerCase().includes(q.toLowerCase())))
        return false;
      return true;
    });
  }, [alerts, q, pf, sf]);
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-border bg-card p-4 flex flex-col md:flex-row gap-3 md:items-center">
        <div className="relative flex-1">
          ...
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by ID, device, or text" className="pl-9 bg-surface/60 border-border" />
        </div>
        <Pills label="Priority" options={priorities} value={pf} onChange={(v) => setPf(v as Priority | "all")} />
        <Pills label="Status" options={statuses} value={sf} onChange={(v) => setSf(v as Status | "all")} />
      </div>
        ...
        {options.map((o) => (
          <button
            ...
            className={cn(
              ...
              value === o ? "bg-khaki text-khaki-foreground font-medium" : "text-muted-foreground hover:text-foreground",
            )}
          >
            {o.replace("_", " ")}
          </button>
